...
72 分講義での実測では、排他制御なしの旧実装（内部ロックで実質直列化）から queue 方式 16 スレッド並列に変更したことで、ASR 処理時間が **106〜175 秒 → 45〜48 秒（2〜3.7 倍高速化）** になりました。16 コアで 2〜3.7 倍に留まるのは、VAD 処理や I/O など ASR 以外のオーバーヘッドが全体時間に占める割合が大きいためです。コア数の少ない環境では RTF がこの数値より大きくなる点にご注意ください。
