...
上記テーブルの fuzzy_sp は初期検証で編集距離閾値を 25% に固定して比較したものです。その後、全パラメータ一括グリッドサーチ（overlap 12段階 × max_dur 5段階 × マージ設定 34通り = 約 2,000 パターン、閾値は 15%〜45% の 7 段階で探索）を実施し、最終的に **overlap=0.90秒、編集距離閾値 15%** が最適と確定しました。
