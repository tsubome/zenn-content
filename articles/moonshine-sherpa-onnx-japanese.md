...
本記事に登場する F1 / Precision / Recall は、**Whisper Large-v3-turbo の認識結果**をリファレンスとした ASR 間比較です。人間による書き起こし（Ground Truth）ではありません。つまり「Moonshine が Whisper の出力にどれだけ一致するか」を測定しており、日本語認識の絶対精度ではない点にご注意ください。Moonshine Base JA の公式 CER（人間リファレンス、FLEURS データセット）は **13.62%** です。
...
- 論文: [arXiv:2509.02523](https://arxiv.org/abs/2509.02523)（**Tiny モデルのみ**を対象）
...
Moonshine（Flavors 世代）には **入力長の上限が約 9 秒** という制約があります（公式ドキュメントには記載がなく、筆者の実験で確認した経験的な値です）。これを超えると ONNX ランタイムが以下のエラーを返します:
...
その後、全パラメータ一括グリッドサーチ（overlap 12段階 × max_dur 5段階 × マージ設定 34通り = 約 2,000 パターン）を実施し、最終的に **overlap=0.90秒、編集距離閾値 15%** が最適と確定しました。
